...
removed shows up in all six. Regenerate the set, don't patch one.
...
- **`goto` the clean URL again — never `reload()`.** By then `syncDealUrl()` has
  ...
  **Assert the overlay reads "Resume game" before shooting** — that is the cheap
  check that the save survived, and without it a run quietly photographs a random
  fresh deal.
- **Draw one card after dismissing the overlay.** A resumed game starts with no
  undo history, so otherwise every shot shows Undo/Redo greyed out. Click the stock
  at `computeLayout(...).stock` rather than at a hard-coded point, so the same code
  works at every viewport.
- **The mid-game boards need a greedy player of their own.** `findHint` used to
  serve as one and is gone; a throwaway "foundation → revealing tableau move →
  waste → draw" loop against the public `Game` API is enough. It is *not* strong
  enough to win a game, though — over 200 seeds its best result is 40 of 52 cards
  home — so `win.png` is the one board assembled directly rather than played:
  A–J home, the queens and kings still out, `parseGameState` asserted on it before
  `restore()`, and the app's own auto-complete left to finish it and start the
  cascade. Eight cards to sweep at +10 and −1 a move lands it on 122 moves / 980.
